    ...
    <>
      <Dragger {...props}>
        <p
          ...
          style={{ height: "10px", marginTop: "0px", padding: "0px" }}
        >
          <UploadOutlined />
        </p>
        <p className="upload-text" style={{ height: "10px" }}>
          Click or drag file to this area to upload
        </p>
      </Dragger>
      <Row style={{ margin: "2px" }}>
        <Col span={8}>
          <Text ellipsis>Report Date</Text>
        </Col>
        <Col span={16}>
          <Input
      ...
      <Row style={{ margin: "2px" }}>
        <Col span={8}>
          <Text ellipsis>Assembly Position</Text>
        </Col>
        <Col span={16}>
          <Input
            placeholder="Assembly Position"
            required
            onChange={(e) => setColAsmPos(e.target.value)}
          />
        </Col>
      </Row>
      <Row style={{ margin: "2px" }}>
        <Col span={8}>
          <Text ellipsis>Assembly Weight</Text>
        </Col>
        <Col span={16}>
          <Input
            placeholder="Assembly Weight"
            required
      ...
      <Row style={{ margin: "2px" }}>
        <Col span={8}>
          <Text ellipsis>Model Total</Text>
        </Col>
        <Col span={16}>
          <Input
            placeholder="Model Total"
            required
            onChange={(e) => setColModelTotal(e.target.value)}
          />
        </Col>
      </Row>
      <Row style={{ margin: "2px" }}>
        <Col span={8}>
          <Text ellipsis>Fabrication Quantity</Text>
        </Col>
        <Col span={16}>
          <Input
            placeholder="Fabrication Quantity"
            required
            onChange={(e) => setColFabQty(e.target.value)}
          />
        </Col>
      </Row>
      <Row style={{ margin: "2px" }}>
        <Col span={8}>
          <Text ellipsis>Fabrication Status</Text>
        </Col>
        <Col span={16}>
          <Input
            placeholder="Fabrication Status"
            required
            onChange={(e) => setColFabStatus(e.target.value)}
          />
        </Col>
      </Row>
      ...
      >
        <Button
          type="primary"
          disabled={colAsmPos === "" || colFabQty === "" || colFabStatus === ""}
          onClick={async () => {
            ...
            rows.every((x) => {
              ...
              if (
                typeof asm_model_total === "undefined" ||
                asm_model_total <= 0
              )
                return true;
              const asm_weight = Number(x[col_index_asm_weight]);
              if (typeof asm_weight === "undefined" || asm_weight <= 0)
                return true;
              const fab_status_in_excel = x[col_index_fab_status];
              const matched_fab_statuses = fabStatuses.filter((x) =>
                x.name.startsWith(fab_status_in_excel)
              );
              if (matched_fab_statuses.length === 0) return true;
              const asm_pos_in_excel = x[col_index_asm_pos];
              if (
                typeof asm_pos_in_excel === "undefined" ||
                asm_pos_in_excel === ""
              )
                return true;
              let fab_qty_in_excel = x[col_index_fab_qty];
              if (
                typeof fab_qty_in_excel === "undefined" ||
                fab_qty_in_excel === ""
              )
                return true;
              const fab_status_id = matched_fab_statuses[0].id;
              object_statuses.push({
                asmPos: asm_pos_in_excel,
                fabQty: fab_qty_in_excel,
                modelTotal: asm_model_total,
                asmWeight: asm_weight,
                fabStatusId: fab_status_id,
              });
              return true;
            });
